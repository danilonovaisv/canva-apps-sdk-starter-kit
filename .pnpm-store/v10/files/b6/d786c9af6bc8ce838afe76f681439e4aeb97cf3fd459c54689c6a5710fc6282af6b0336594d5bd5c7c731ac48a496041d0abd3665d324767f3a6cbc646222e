...
exports.buildIntlShape = buildIntlShape;
const intl_1 = require("@formatjs/intl");
const pseudolocalize_1 = require("../components/pseudolocalize");
 ...
 */
function buildIntlShape(locale, messages, config, pseudolocalize) {
    ...
    const intlShape = (0, intl_1.createIntl)({
        defaultLocale: "en",
        locale,
        messages,
        ...config,
    ...
    if (isPseudolocaleEnabled && pseudolocalize) {
        intlShape.formatMessage = (0, pseudolocalize_1.createFormatMessage)(locale, pseudolocalize);
    }
    return intlShape;
}
